...
- **Map (Part 3) — Mapbox, as a header on My Bridges (NOT a tab).** The tab bar stays 3 tabs (Search · My Bridges · Stats). My Bridges is now: **header (unchanged) → map (full-width, ~38dvh, fixed) → bridge list (scrolls independently below)**. Default style **`streets-v12`**, with **style-switcher pills** overlaid top-right — **Streets / Satellite (`satellite-streets-v12`) / Terrain (`outdoors-v12`)** — that swap the basemap instantly (re-adding pins/clusters/3D after `setStyle`); pins colored by structure type from `structureTypes.ts` (one source of truth — hybrids use the primary/first type; popup shows the full badge); built-in GL **clustering** (tap → zoom); tap pin → **callout (name + badge) → tap → bridge detail**; **fitBounds** to her pins, **continental-US** fallback when none. Map always shows (logged-out = US + "Log in" prompt below; logged-in no-bridges = US + empty state + "Find a bridge" → Search; logged-in = her pins + cards). **3D buildings** (fill-extrusion, brand-navy) rise at **zoom 15+**, with an auto-tilt (pitch 45°, bearing −17.6° on `zoomend`) so the overview stays flat but zooming into a city brings the streetscape alive around the pins. `VITE_MAPBOX_TOKEN` from env (graceful "Map unavailable" placeholder if absent). `mapbox-gl` is **lazy-loaded** (code-split chunk, ~482 KB gzip) so it only loads when My Bridges opens.
- **My Bridges structure-type filter (2026-06-19).** A "Filter by type" button **below the map** (collapsed by default) reveals colored badge-chips for **only the structure types present in her collection**; selecting narrows both the cards and the map pins (OR within types). This is fresh, scoped to `LoggedBridge[]` — the deleted Phase 1.5 `FilterControls`/`lib/filters` was Search-coupled/multi-facet/GPS-aware and was *not* restored; only the ~4-line structure-type match logic was reused.
...
**Caching rationale:** Rather than hitting Overpass + Wikipedia on every view, cache bridge data in Supabase. **As built (2026-06-19), the search/browse list does NOT enrich — it renders Photon-only shallow results immediately (~0.6s vs the old ~30s).** Enrichment (Overpass→Wikidata→Wikipedia) is deferred to **on-open**: opening a bridge does a `bridge_cache` **read-through** (fresh <30 days → instant, zero APIs), else enriches just that one bridge (~2–3s) and writes it back to `bridge_cache` (best-effort; RLS allows insert only for signed-in users). The list also does one batched `bridge_cache` read to show badges for already-cached results. Rows are still never *updated* client-side (insert-only), so the 30-day refresh of an existing stale row remains a future service-role job. (Logged crossings still write the cache too.)
