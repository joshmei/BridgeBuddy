...
> **Latest change (2026-06-13, session 5):** **"I've Crossed This" → "Crossed! ✓" + Undo, with SOFT DELETE.** Once crossed, the button becomes plain "Crossed! ✓" text plus a small muted-rose **Undo** button (no confirmation). Removal is a **soft delete** — Undo sets `is_deleted = true` (or decrements `crossing_count` if > 1); rows are never hard-deleted (preserved for a future "Recently Removed"/recovery). All reads filter `is_deleted = false`; re-crossing revives the row. New `is_deleted` column (already added in Supabase; recorded in `0004_user_logs_soft_delete.sql`). The earlier hard-delete policy (`0003`) is now unused. **Earlier in Phase 2.6 (verified live):** Welcome screen with the real drone hero video (transcoded 183MB→8.4MB), flash-free poster/navy load, and PWA / Add-to-Home-Screen icons + manifest. **Next: Phase 3 — Map + stats** (Mapbox — user getting an API key).
