...
> Living document. Update this as decisions are made. Last updated: 2026-06-13 (session 5).
> **Working name:** Bridge Buddy (placeholder — real name still TBD, see §10/§11).
> **Latest change (2026-06-13, session 5):** **"I've Crossed This" → "Crossed! ✓" + Undo.** Once crossed, the button becomes plain "Crossed! ✓" text plus a small muted-rose **Undo** button that deletes the log (no confirmation) and restores the button — added the owner-scoped `delete` RLS policy (`0003_user_logs_delete.sql`, **paste in Supabase**). Note: this removes the re-tap-to-increment path, so `crossing_count` stays 1 from the UI for now (see §5.5 Phase 2). **Earlier in Phase 2.6 (verified live):** Welcome screen with the real drone hero video (transcoded 183MB→8.4MB), first-frame poster + navy base for a flash-free open, and PWA / Add-to-Home-Screen icons + manifest. **Next: Phase 3 — Map + stats** (Mapbox — user getting an API key).
...
- **Deliverable:**
  - **"I've Crossed This"** button on the detail page. Tap → log with `first_recorded_crossing = last_crossing = today`, `crossing_count = 1`. ("First recorded" is honest — it's the first time she logged it *in this app*, not a claim about her true first-ever crossing.) **Crossed-state UI revised 2026-06-13 (§ below):** once crossed, the button is replaced by plain **"Crossed! ✓"** text + a small muted-rose **Undo** button that *deletes* the log (no confirmation) and restores the original button. **Consequence:** there's no longer a re-tap-to-increment affordance, so `crossing_count` stays 1 from the UI (the increment logic + column remain for a future "log another crossing" action; "Crossed X times" / most-crossed-by-count are dormant until then).
...
| 2 | Map library | **Detail-page single pin: OSM embed iframe (dependency-free) — DECIDED 2026-05-29.** Full multi-pin map: **Mapbox — DECIDED 2026-06-13** (over Leaflet+OSM and Leaflet+CARTO), for the polished vector rendering + a custom dark style matched to the `#0f1e35` brand with colored pins. Needs a Mapbox account + public access token (user creating it). Lands in **Phase 3**. | Phase 3 |
...
- **Supabase Row Level Security — before Phase 2 ships. AS BUILT 2026-05-30 (`supabase/migrations/0002_rls.sql`).** RLS is **enabled on both tables** (`bridge_cache`, `user_logs`); there is no `public.users` table (auth.users is managed by Supabase and not anon-exposed). Supabase tables are publicly reachable via the anon key, so without RLS any client could read or write every user's logs.
  - `user_logs` — owner-scoped: `select` / `insert` / `update` / `delete` policies all require `user_id = auth.uid()`. (Delete added 2026-06-13 in `0003_user_logs_delete.sql` for the "Undo" action.) No user can see or touch another user's logs.
